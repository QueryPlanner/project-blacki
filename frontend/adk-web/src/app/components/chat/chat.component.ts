...
import {
  AGENT_SERVICE,
  OpenRouterModel,
} from '../../core/services/interfaces/agent';
...
export class ChatComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  selectedModel: WritableSignal<string | null> = signal(null);
  availableModels: WritableSignal<OpenRouterModel[]> = signal([]);
  ...
  protected loadModels(): void {
    ...
    this.agentService.listModels().subscribe({
      next: (models) => {
        this.availableModels.set(models);
        if (models.length > 0 && this.selectedModel() === null) {
  ...
  async sendMessage(event: Event) {
    ...
    const req: AgentRunRequest = {
      ...
      stateDelta: this.buildStateDeltaWithSelectedModel(),
    };
    this.selectedFiles = [];
  ...
  private buildStateDeltaWithSelectedModel(): any {
    const selected = this.selectedModel();
    const existingStateDelta = this.updatedSessionState();

    if (!selected) {
      if (!existingStateDelta || typeof existingStateDelta !== 'object') {
        return undefined;
      }
      const {selectedModel: _, ...rest} = existingStateDelta as Record<string, unknown>;
