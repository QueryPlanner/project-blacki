...
import {
  AgentService as AgentServiceInterface,
  OpenRouterModel,
...
export class AgentService implements AgentServiceInterface {
  ...
  listModels(): Observable<OpenRouterModel[]> {
    if (this.apiServerDomain != undefined) {
      const url = this.apiServerDomain + `/models`;
      return this.http
        .get<{models: OpenRouterModel[]; error?: string}>(url)
        .pipe(
          switchMap((res) => {
            if (res.error) {
              console.warn('Failed to fetch models:', res.error);
            }
            return of(res.models ?? []);
          }),
          catchError((err) => {
