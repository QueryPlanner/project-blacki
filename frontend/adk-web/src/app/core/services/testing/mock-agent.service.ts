...
import {OpenRouterModel} from '../interfaces/agent';
...
export class MockAgentService implements Partial<AgentService> {
  ...
  listModelsResponse = new ReplaySubject<OpenRouterModel[]>(1);

  constructor() {
    this.listModelsResponse.next([]);
  }
  ...
  listModels = jasmine
    ...
    .and.returnValue(this.listModelsResponse);
}
