...
describe('AgentService', () => {
  ...
  describe('listModels', () => {
    it('should call /models endpoint and return models', async () => {
      const modelsPromise = firstValueFrom(service.listModels());
      const req = httpTestingController.expectOne(
          API_SERVER_BASE_URL + '/models',
      );
      expect(req.request.method).toEqual(METHOD_GET);
      req.flush({
        models: [
          {id: 'openrouter/stepfun/step-3.5-flash:free', name: 'Step 3.5 Flash'},
        ],
      });
    ...
    it('should return empty array when apiServerDomain is undefined', async () => {
      (service as {apiServerDomain: string}).apiServerDomain = undefined as unknown as string;
      const models = await firstValueFrom(service.listModels());
      expect(models).toEqual([]);
    });
  });
