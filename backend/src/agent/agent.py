...
# Load .env before reading ROOT_AGENT_MODEL. The agent module can be imported
# before the server's initialize_environment runs (e.g. when running
# `python -m agent.server`), so we must load env vars here for model config.
